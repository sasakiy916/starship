...
msl_a = [0]*MISSILE_MAX  # 弾が飛んでいく角度のリスト
...
EMY_ZAKO = 1  # ザコ敵の番号を管理する定数
EMY_BOSS = 5
LINE_T = -80  # 敵が出現(消失)する上端の座標
LINE_B = 800
LINE_L = -80
LINE_R = 1040
...
def bring_enemy():
    ...
    if tmr % 30 == 0:
        if 0 < sec < 15 and tmr % 60 == 0:
            set_enemy(random.randint(20, 940), LINE_T,
                      90, EMY_ZAKO, 8, 1)  # 敵1
            set_enemy(random.randint(20, 940), LINE_T,
                      90, EMY_ZAKO+1, 12, 1)  # 敵2
            set_enemy(random.randint(100, 860), LINE_T,
                      random.randint(60, 120), EMY_ZAKO+2, 6, 3)  # 敵3
            set_enemy(random.randint(100, 940), LINE_T,
                      90, EMY_ZAKO+3, 12, 2)  # 敵4
        if tmr == 30 * 20:  # ボス出現
        ...
        if emy_f[i] == True:
            ...
            if emy_type[i] < EMY_BOSS:
                emy_x[i] = emy_x[i]+emy_speed[i] * \
                    math.cos(math.radians(emy_a[i]))
                emy_y[i] = emy_y[i]+emy_speed[i] * \
                    math.sin(math.radians(emy_a[i]))
                ...
                if emy_shield[i] < 100 and tmr % 30 == 0:
                    set_enemy(emy_x[i], emy_y[i]+80,
                              random.randint(60, 120), EMY_BULLET, 6, 0)
                    ...
                    if msl_f[n] == True and get_dis(emy_x[i], emy_y[i], msl_x[n], msl_y[n]) < r**2:
                        ...
                        if emy_shield[i] == 0:
                            ...
                            if emy_type[i] == EMY_BOSS and idx == 1:
                                ...
                                for j in range(10):
                                    set_effect(
                                        emy_x[i]+random.randint(-er, er), emy_y[i]+random.randint(-er, er))
                                    se_explosion.play()
...
def main():  # メインループ
    ...
    while True:
        ...
        if idx == 0:  # タイトル
            ...
            screen.blit(img_rz, [480-img_rz.get_width() /
                        2, 280-img_rz.get_height()/2])
        ...
        if idx == 2:  # ゲームオーバー
            ...
            if tmr <= 90:
                if tmr % 5 == 0:
                    set_effect(ss_x+random.randint(-60, 60),
                               ss_y+random.randint(-60, 60))
                if tmr % 10 == 0:
                    se_damage.play()
            if tmr == 120:
        ...
        draw_effect(screen)
        draw_text(screen, f"SCORE {str(score)}", 200, 30, 50, SILVER)
        if idx != 0:  # シールド表示
            screen.blit(img_shield, [40, 680])
            pygame.draw.rect(screen, (64, 32, 32), [
